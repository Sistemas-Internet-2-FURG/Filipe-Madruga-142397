...
class records(base):    
  ...
  def fetchone_classes_students_classcode(self, classcode):
    with self.connect() as conn:
      classes_students = conn.execute("SELECT * FROM classes_students WHERE classcode == ?", (classcode.strip(),))
    return classes_students.fetchone()
  
  def fetchone_classes_students_registration(self, registration):
    with self.connect() as conn:
      classes_students = conn.execute("SELECT * FROM classes_students WHERE registration == ?", (registration.strip(),))
    return classes_students.fetchone()
